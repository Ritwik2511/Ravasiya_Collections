...
class CSCPicker extends flutter.StatefulWidget {
  ...
  @override
  flutter.State<CSCPicker> createState() => CSCPickerState();
}

class CSCPickerState extends flutter.State<CSCPicker> {
  List<String?> _cities = [];
  ...
  void initState() {
    ...
    Future.delayed(Duration(seconds: 1),() {
      countries!.forEach((element) {
        ...
        if (element.name == widget.countryDropdownLabel) {
  ...
  Future<List<String?>> getCountries() async {
    _country.clear();
    countries = productDataController.countrieList;
  ...
  void addCountryToList(Country data) {
    if (!mounted) return;
    setState(() {
      _country.add(data.name);
    });
  }
  ...
  Future<List<String?>> getStates() async {
    _states.clear();
    countries!.forEach((element) {
      if (element.name == _selectedCountry) {
        element.states!.forEach((state) {
          setState(() {
            _states.add(state.name);
          });
      ...
      children: [
        widget.layout == Layout.vertical
          ...
          children: [
            ...
            _states.isNotEmpty ? stateDropdown() : Container(),
            // getVerSpace(16.h),
            // widget.showStates && widget.showCities
            //     ? cityDropdown()
            //     : Container()
